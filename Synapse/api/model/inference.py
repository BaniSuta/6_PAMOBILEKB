...
class Model:
    ...
    def __init__(self, model_path):
        if model_path.endswith(".pkl"):
            ...
            raise ValueError(
                f"Model format '{model_path.split('.')[-1]}' not supported. Please use '.pkl', '.joblib', '.h5', or '.tflite'."
            )

    def ahp_ranking(self):
        ranking = self.model["ranking"]
        if isinstance(ranking, np.ndarray):
            ranking = ranking.tolist()  # Konversi ke list jika masih berupa NumPy array
    ...
    def data_pipeline(self, numerical_features=None, scaler_type="standard"):
        ...
        if numerical_features:
            if scaler_type == "standard":
                ...
                raise ValueError(
                    f"Unsupported scaler type: '{scaler_type}'. Use 'standard' or 'minmax'."
                )
    ...
    def predict_from_data(self, data, numerical_features=None):
        ...
        if self.model_type == "sklearn":
            ...
            prediction = (
                "setosa"
                if prediction == 0
                else "versicolor" if prediction == 1 else "virginica"
            )
